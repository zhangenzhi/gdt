...
def visualize_predictions(epoch, output_dir, save_dir, images, targets, preds_logits, val_loader, num_classes=5):
    ...
    fig.suptitle(f'Epoch {epoch + 1} Validation Results (SAM S8D)', fontsize=20) # 更新标题
...
def train(config, args):
    ...
    if is_main_process:
        ...
        logging.info(f"开始 SAM S8D 训练，使用 {world_size} 个进程...") # 更新日志
    ...
    # --- 模型创建 ---
    # !! 修改: 创建 SAM 模型 !!
    if is_main_process: logging.info(f"创建模型: SAM-B Variant for Segmentation")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SAM-B S8D Segmentation Training Script")
    parser.add_argument('--config', type=str, default='./configs/sam-b_S8D.yaml', help='配置文件路径') # 默认 SAM S8D 配置
    ...
    if args.savefile: config.setdefault('output', {})['save_dir'] = args.savefile
        ...
        config['output']['save_dir'] = config['output'].get('save_dir', config.get('task_name', 'sam_s8d_run'))

    # 设置 args
    args.data_dir = config.get('data', {}).get('data_dir', './data_s8d')
    args.output = config.get('output', {}).get('base_dir', './output_s8d')
    args.savefile = config.get('output', {}).get('save_dir', 'sam_s8d_run')
